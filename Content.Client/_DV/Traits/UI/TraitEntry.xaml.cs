...
using Content.Shared._Mono.Traits.Conditions;
...
public sealed partial class TraitEntry : PanelContainer
{
    ...
    public void UpdateConditionsMet(
        ...
        string? companyName // Mono - Company conditions
    )
    {
        ...
        foreach (var condition in _trait.Conditions)
        {
            var result = condition switch
            {
                ...
                AnyOfCondition anyOfCond => CheckAnyOfCondition(anyOfCond, jobId, speciesId, antagPreferences, companyName),
                InCompanyCondition inCompanyCond => CheckInCompanyCondition(inCompanyCond, companyName), // Mono: Companies
                _ => true,
            };
    ...
    )
    {
        ...
        foreach (var childCondition in condition.Conditions)
        {
            var result = childCondition switch
            {
                ...
                AnyOfCondition nestedAnyOf => CheckAnyOfCondition(nestedAnyOf, jobId, speciesId, antagPreferences, companyName), // Recursive!
                IsAntagEligibleCondition antagEligibleCond => CheckAntagEligibleCondition(antagEligibleCond, antagPreferences),
                InCompanyCondition inCompanyCond => CheckInCompanyCondition(inCompanyCond, companyName), // Mono - Company conditions
                _ => true,
            };
