## UI
door-remote-open-close-text = Abre y cierra puertas
door-remote-toggle-bolt-text = Activa/desactiva los cerrojos
door-remote-emergency-access-text = Activa/desactiva el acceso de emergencia
door-remote-invalid-text = No válido
door-remote-mode-label = Modo: [color=white]{$modeString}[/color]

## Entity

door-remote-switch-state-open-close = Cambias el mando para abrir y cerrar puertas
door-remote-switch-state-toggle-bolts = Cambias el mando para activar/desactivar los cerrojos
door-remote-switch-state-toggle-emergency-access = Cambias el mando para activar/desactivar el acceso de emergencia
door-remote-no-power = La puerta no tiene energía
door-remote-denied = Acceso denegado

